...
4. Include modified files and a durable report path when applicable. After
   `worker_done`, end the dispatched turn and idle; do not poll or start new work.
...
```text
ORCA orchestration send --from <worker_handle> --dispatch-capability <capability> --type worker_done --subject "<short status>" --body "<three sentences: work, findings, remaining>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a,path/b" --report-path "<optional report>"
```
