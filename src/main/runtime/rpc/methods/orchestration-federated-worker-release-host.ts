...
import type { inspectRemoteAttachment } from './orchestration-federation-attachment-observation'

const remoteArchiveResourceId = (dispatchId: string): string => `remote-attachment:${dispatchId}`
...
export async function readRemoteAttachmentArchive(args: {
  ...
  return readArchivedWorkerOutput({
    ...
    resource: {
      id: remoteArchiveResourceId(args.attachment.dispatch_id),
    ...
    try {
      ...
      return {
        ...
        archive: archiveSummary(
          db.markWorkerTerminalReleaseUnknown(
            resource.id,
