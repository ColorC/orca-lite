  ...
  const results = await mapWithConcurrency(groups, FLEET_HOST_CONCURRENCY, async (group) => {
    ...
    try {
      ...
      for (const dispatch of group.dispatches) {
        args.db.updateFederatedDispatchRuntimeEpoch(dispatch.dispatch_id, capability.runtimeEpoch)
      }
      ...
      for (const dispatch of group.dispatches) {
        args.db.updateFederatedDispatchRuntimeEpoch(dispatch.dispatch_id, snapshot.runtimeEpoch)
      }
      const expected = new Set(dispatchIds)
      return {
        observations: snapshot.items.filter((item) => expected.has(item.dispatchId)),
        error: null
      }
    } catch (caught) {
