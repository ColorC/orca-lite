...
describe('hydrateEditorSession', () => {
  ...
  // Why the same path is open twice: the preview's id is minted from its path, so the two tabs are
  // distinct surfaces on one document — and the edit tab's own id must not drift because of it.
  it('restores an HTML preview under the id its tab chrome names', () => {
    const store = createTestStore()
    const wt = 'repo1::/path/wt1'
    ...
    store.getState().hydrateEditorSession({
      ...
      activeFileIdByWorktree: { [wt]: `html-preview::${wt}::${filePath}` },
      activeTabTypeByWorktree: { [wt]: 'editor' }
    })

    const s = store.getState()
    expect(s.openFiles).toHaveLength(2)
    expect(s.openFiles[0]).toMatchObject({ id: filePath, mode: 'edit' })
    expect(s.openFiles[1]).toMatchObject({
      id: `html-preview::${wt}::${filePath}`,
      filePath,
      relativePath: 'report/index.html',
      worktreeId: wt,
      mode: 'html-preview',
    ...
    // Why the active id too: the preview was the surface in front of the reader at shutdown.
    expect(s.activeFileId).toBe(`html-preview::${wt}::${filePath}`)
    expect(s.activeTabType).toBe('editor')
  })
